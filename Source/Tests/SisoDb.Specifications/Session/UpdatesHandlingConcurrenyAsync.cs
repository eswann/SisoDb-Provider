...
namespace SisoDb.Specifications.Session
{
    class UpdatesHandlingConcurrenyAsync
    {
        ...
        public class when_item_has_enabled_concurrency_check_and_update_async_is_not_latest : SpecificationBase
        {
            ...
            Because of = () =>
            {
                using (var session = TestContext.Database.BeginSession())
                {
                    ...
                    session.UpdateAsync(copy1).Wait();
        ...
        [Subject(typeof(ISession), "Update (concurrencies)")]
        public class when_item_has_enabled_int_concurrency_check_and_update_async_is_not_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
        ...
        [Subject(typeof(ISession), "Update (concurrencies)")]
        public class when_item_has_enabled_long_concurrency_check_and_update_async_is_not_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
        ...
        [Subject(typeof(ISession), "Update (concurrencies)")]
        public class when_item_has_enabled_concurrency_check_and_update_async_is_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
        ...
        [Subject(typeof(ISession), "Update (concurrencies)")]
        public class when_item_has_enabled_int_concurrency_check_and_update_async_is_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
        ...
        [Subject(typeof(ISession), "Update (concurrencies)")]
        public class when_item_has_enabled_long_concurrency_check_and_update_async_is_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
        ...
        public class when_updating_using_inline_modifier_and_item_has_enabled_concurrency_check_and_update_async_is_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
            ...
            Because of = () =>
            {
                using (var session = TestContext.Database.BeginSession())
                {
                    session.UpdateAsync<ModelWithGuidToken>(_orgItem.Id, x =>
                    {
                        x.StringValue = "From first update";
                        x.IntValue = 142;
                    }).Wait();
                    session.UpdateAsync<ModelWithGuidToken>(_orgItem.Id, x => x.StringValue = "From second update").Wait();
                }
            };
        ...
        [Subject(typeof(ISession), "Update inline (concurrencies)")]
        public class when_updating_interface_using_inline_modifier_and_item_has_enabled_concurrency_check_and_update_async_is_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
            ...
            Because of = () =>
            {
                using (var session = TestContext.Database.BeginSession())
                {
                    session.UpdateAsync<IModelWithGuidToken, ModelWithGuidToken>(_orgItem.Id, x =>
                    {
                        x.StringValue = "From first update";
                        x.IntValue = 142;
                    }).Wait();
                    session.UpdateAsync<IModelWithGuidToken, ModelWithGuidToken>(_orgItem.Id, x => x.StringValue = "From second update").Wait();
                }
            };
        ...
        [Subject(typeof(ISession), "Update inline (concurrencies)")]
        public class when_updating_using_inline_modifier_and_item_has_enabled_concurrency_check_and_update_async_is_latest_but_process_is_manually_aborted : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
            ...
            Because of = () =>
            {
                using (var session = TestContext.Database.BeginSession())
                {
                    ...
                    session.UpdateAsync<ModelWithGuidToken>(_orgItem.Id, x => x.StringValue = "From second update", x => false).Wait();
                }
            };
            ...
            Because of = () =>
            {
                using (var semaphore = new SemaphoreSlim(1, 1))
                {
                    ...
                    try
                    {
                        ...
                        {
                            semaphore.Wait();
                            CaughtException = Catch.Exception(() => TestContext.Database.UseOnceTo().UpdateAsync<ModelWithGuidToken>(_orgItem.Id, x => x.StringValue = "I am a nasty concurrent update.").Wait());
                            semaphore.Release();
                        });
        ...
        [Subject(typeof(ISession), "Update (concurrencies)")]
        public class when_item_is_interface_and_has_enabled_concurrency_check_and_update_async_is_latest : SpecificationBase
        {
            Establish context = () =>
            {
                TestContext = TestContextFactory.Create();
            ...
            Because of = () =>
            {
                using (var session = TestContext.Database.BeginSession())
                {
                    ...
                    _copy1.StringValue = "From copy 1";
                    session.UpdateAsync<IModelWithGuidToken>(_copy1).Wait();

                    _copy2 = session.GetByIdAs<IModelWithGuidToken, ModelWithGuidToken>(_orgItem.Id);
                    _copy2.StringValue = "From copy 2";
                    session.UpdateAsync<IModelWithGuidToken>(_copy2).Wait();
                }
            };
